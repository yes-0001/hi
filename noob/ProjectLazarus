...
Metatable.__namecall = function(self, ...)
    ...
    local Args = {...}
    if Settings.Instakill and Method == "FireServer" and tostring(self) == "Damage" then
	    Args[1].Damage = 9999
        return Namecall(self, unpack(Args))
    end
    if Settings.Wallbang and Method == "FindPartOnRayWithIgnoreList" then
        table.insert(Args[2], Map)
        table.insert(Args[2], Interact)
        return Namecall(self, unpack(Args))
    end
    if Method == "FireServer" and self == SendData then -- anti cheat bypass btw
    	return
    end
